...
TEST_F(ParserTest, illegal1) {
  ...
  try{
     ...
     EXPECT_EQ("unexpected token",err);
  }
}
...
TEST_F(ParserTest, ListAsStruct) {
  ...
  ASSERT_EQ("1", terms->args(0)->symbol());
  ASSERT_EQ("[]", terms->args(1)->symbol());
}
...
TEST_F(ParserTest, ListAsStruct2) {
  ...
  ASSERT_EQ("2", terms->args(0)->symbol());
  ASSERT_EQ(".(1, [])", terms->args(1)->symbol());
}
