...
TEST_F (ScannerTest, position) {
  ...
  EXPECT_EQ(0, scanner.position()); //init position

  EXPECT_EQ(1, scanner.skipLeadingWhiteSpace()); //跳過空白
  EXPECT_TRUE(isdigit(scanner.currentChar())); //currentChar 回傳當前符號 isdigit是否為數字
  EXPECT_EQ(12345, scanner.extractNumber()); //提出數字、並往前進
  ...
  EXPECT_EQ("tom", scanner.extractAtom()); //提出數字
